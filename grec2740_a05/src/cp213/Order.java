...
public class Order implements Printable {
	...
	if (items.containsKey(item)) {
	    ...
	    // else adding to items with given quantity
	...
	for (MenuItem item : items.keySet()) {
	    BigDecimal price = item.getPrice().multiply(new BigDecimal(items.get(item)));
	...
	// your code here
	BigDecimal result = getSubTotal().multiply(TAX_RATE);
	return result;
    }
	...
	// your code here
	BigDecimal result = getSubTotal().add(getTaxes());
	return result;
    }
	...
	for (MenuItem item : items.keySet()) {
	    // appending to String returned from item " x " followed by quantity and a
	    // newline
	    // str += item.toString() + " x " + items.get(item) + " \n";
	    str += String.format("%-13s %d @ $%,5.2f = $%,5.2f\n", item.getName(), items.get(item), item.getPrice(),
		    (item.getPrice().multiply(new BigDecimal(items.get(item)))));
	}
	str += String.format("Subtotal:                   $ %.2f\n", getSubTotal());
	str += String.format("Taxes:                      $ %.2f\n", getTaxes());
	str += String.format("Total:                      $ %.2f", getTotal());
	return str;
    }
