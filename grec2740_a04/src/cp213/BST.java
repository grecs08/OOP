...
public class BST<T extends Comparable<T>> {
	...
	if (source != null && target != null) {
	    ...
	    isEqual = false;
	} else if (source.getCs().getDatum() != target.getCs().getDatum()) {
	    isEqual = false;
		...
		isEqual = this.equalsAux(source.getRight(), target.getRight());
	    }
	}
	return isEqual;
    }
	...
	if (node == null) {
		...
		- 1) {
	    valid = false;
	} else if (node.getLeft() != null && node.getLeft().getCs().getDatum().compareTo(node.getCs().getDatum()) >= 0
		|| node.getRight() != null
			&& node.getRight().getCs().getDatum().compareTo(node.getCs().getDatum()) <= 0) {
	    valid = false;
	} else {
	    valid = this.isValidAux(node.getLeft(), null, null) && this.isValidAux(node.getRight(), null, null);
	}
	return valid;
    }
